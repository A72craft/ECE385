...
#define MAX3421E_SLAVE_MASK 0x00000000
#define MAX3421E_SLAVE_MASK_OFF 0x00000001
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//write reg + 2 via SPI
	Status = XSpi_Transfer(&SpiInstance, sendData, NULL, 2);
	if(Status != XST_SUCCESS) {
		xil_printf("Error: SPI transfer failed in MAXreg_wr. Status %d\r\n", Status);
		return 0;
    }

	//deselect MAX3421E (may not necessary if you are using SPI peripheral)
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	Status = XSpi_Transfer(&SpiInstance, sendData, NULL, nbytes+1);
	if(Status != XST_SUCCESS) {
		xil_printf("Error: SPI transfer failed in MAXbytes_wr. Status %d\r\n", Status);
    }
...
BYTE MAXreg_rd(BYTE reg) {
	...
	Status = XSpi_Transfer(&SpiInstance, sendReg, readData, 2);
	if(Status != XST_SUCCESS) {
		xil_printf("Error: SPI transfer failed at MAXreg_rd. Status %d\r\n", Status);
		return 0;
    }
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	// Following bytes can be dummy data, as we just need to generate clock pulses to read the data
	for(i = 1; i <= nbytes; i++) {
		sendData[i] = 0; // Set dummy data
	}
	//select MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance,1);

	//read data[n] from SPI, where n goes from 0 to nbytes-1
	Status = XSpi_Transfer(&SpiInstance, sendData, readData, nbytes + 1);
	if(Status != XST_SUCCESS) {
		xil_printf("Error: SPI transfer failed at MAXbytes_rd. Status %d\r\n", Status);
		return 0;
    }
